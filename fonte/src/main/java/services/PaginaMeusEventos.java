...
public class PaginaMeusEventos {
	...
	public static String createMeusEventos(Request req, Response res) {
		...
		for (int i = 0; i < meusEventos.size(); i++) {
			...
			ul += "<div>\n";
			ul += "<a href=\"/updateEvento/" + meusEventos.get(i).getId() + "\">"
					+ "<i class=\"fa-solid bi bi-pencil-square\"></i>" + "</a>\n";
			ul += "<a href=\"/deleteEvento/" + meusEventos.get(i).getId() + "\">"
		...
		list += "</div>\n";
		page = page.replaceFirst("<LOGIN-BUTTON>",
				"<li class=\"nav-item\"><a id=\"user-view\"><i class=\"fa-solid fa-circle-user\"></i></a></li>");
